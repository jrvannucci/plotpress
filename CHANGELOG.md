...
- **An explicit, non-positive limit on a log axis now raises, instead of
  ...
  A one-sided limit (the open end left to autoscale, e.g. `set_ylim(bottom=
  -5)`) used to slip past this same check and still silently blank the
  axis -- it is now validated per end, not only when both ends are set.
...
- **`get_cmap()`/`register_cmap()` no longer hand out the shared colormap
  array by reference.** Mutating a value returned from either function, or
  a LUT already passed to `register_cmap()`, used to silently corrupt that
  colormap for every other figure/artist in the process; both now always
  return/store a copy, on every path (registering a colormap and then
  mutating what it handed back used to still corrupt the registry).
...
- **`hist()` now ignores non-finite values**, matching every sibling
  ...
  NaN in the data. It also now warns when a dataset had no finite values at
  all and was dropped entirely, matching `boxplot()`/`violinplot()` below.
...
- **`Figure.print_layout_summary()`/`Axes.print_summary()` no longer report
  ...
  `Axes.print_summary()` now also flags when there is a figure-level export
  gap it can't attribute to that specific axes, rather than only checking
  its own axes and missing one it can't name.

### Documentation

- **Several docs gallery scripts and cross-references still pointed at
  pre-reorganization module paths** (`plotpress.raster`, `plotpress.
  artists`, and stale `:mod:`/`:class:`/`:func:` roles across several
  docstrings) after the moves above. None were caught by this session's own
  earlier docs-build checks, because sphinx-gallery caches each script's
  execution by a hash of its own source text -- a script whose only problem
  was its import *target* moving elsewhere never re-executed, and kept
  reusing its stale pre-move result build after build. Fixed, and confirmed
  this time with every gallery cache wiped first so every script actually
  re-ran.
