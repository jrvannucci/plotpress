...
|---|---|---|
| `plotpress/figure.py` | 4540 | The root object: layout, `to_svg`/`to_html`/`save`/`show`, figure-level text and legend, grouping (`Group`/`GroupLayout`/`subplots_from_groups`), the HTML round-trip (`load_data`, `subplots_from_html`), the data-free template round-trip (`Figure.to_template`/`save_template`, `load_template`, `figure_from_template`), and the `Report` multi-figure aggregator |
...
| `plotpress/svg.py` | 3031 | SVG serialization — one `_render_*` per artist kind, plus axis decoration; also the source of several private helpers `raster.py` imports (legend/tick/text-box geometry) |
