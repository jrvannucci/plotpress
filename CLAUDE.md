...
|---|---|---|
| `plotpress/figure.py` | 4406 | The root object: layout, `to_svg`/`to_html`/`save`/`show`, figure-level text and legend, grouping (`Group`/`GroupLayout`/`subplots_from_groups`), the template round-trip (`load_data`/`Figure.to_template`/`save_template`/`load_template` all producing the same shape, `figure_from_template` the one function that rebuilds a figure from it, whether recovered from an HTML export or built standalone), and the `Report` multi-figure aggregator |
