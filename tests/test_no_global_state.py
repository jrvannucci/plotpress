"""plotpress's defining property: no global state. Figures are fully independent."""
...
def test_no_pyplot_or_current_figure():
    ...
    assert not hasattr(plotpress, "pyplot")
    assert not hasattr(plotpress, "gcf")
    assert not hasattr(plotpress, "gca")
    assert not hasattr(plotpress, "rcParams")
...
def test_norm_with_explicit_limits_still_puts_artists_on_one_scale():
    shared = plotpress.Normalize(0, 100)
...
@pytest.mark.parametrize("norm,attr,value", [
    (plotpress.LogNorm(), None, None),
    (plotpress.PowerNorm(0.4), "gamma", 0.4),
    (plotpress.SymLogNorm(1.5), "linthresh", 1.5),
])
